...
def _validate_section_patches(skill_name: str, section_patches: list[dict]) -> list[str]:
    ...
    for idx, section in enumerate(section_patches):
        ...
        for p_idx, patch in enumerate(patches):
            ...
            # Allow insert-only patches for auto doc sections (new section body for a newly created heading).
            if not (before or old or after):
                if heading.startswith("## ADK: ") and new.strip():
                    continue
    ...
    for item in skills:
        ...
        if item.get("new_skill") and item.get("full_content"):
            ...
            subprocess.run(
                [
                    sys.executable,
                    str(scripts_dir / "apply_skill_updates.py"),
                    str(skill_dir),
                    updates_json,
                    "--allow-new-sections",
                ],
